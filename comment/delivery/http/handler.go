...
func (h *Handler) NewComment(w http.ResponseWriter, r *http.Request) {
	...
	comments, err := h.usecase.Create(models.Comment{
		UserID: id,
		PostId: p.Postid,
		Text:   p.Text,
	})
...
func commentsMarshal(allcomment []models.Comment) ([]byte, error) {
	jsonComments := toComments(allcomment)
	return json.Marshal(&jsonComments)
}

func toComment(v models.Comment) outputComment {
...
func toComments(allcomment []models.Comment) allOutputComment {
